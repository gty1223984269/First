...
namespace DTcms.DAL
{
    ...
    public partial class article
    {
        ...
        public int Add(Model.article model)
        {
            ...
            strSql.Append("channel_id,category_id,call_index,title,link_url,img_url,seo_title,seo_keywords,seo_description,zhaiyao,content,sort_id,click,status,groupids_view,vote_id,is_top,is_red,is_hot,is_slide,is_sys,is_msg,user_name,add_time,update_time,SchoolId,CityId)");
            ...
            parameters[26].Value = model.ScholId;
            parameters[27].Value = model.CityId;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,channel_id,category_id,call_index,title,link_url,img_url,seo_title,seo_keywords,seo_description,zhaiyao,content,sort_id,click,status,groupids_view,vote_id,is_top,is_red,is_hot,is_slide,is_sys,is_msg,user_name,add_time,update_time,Sstatus");
            strSql.Append(" from " + databaseprefix + "article ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
        ...
        /// 获得查询分页数据
        /// </summary>
        public DataSet GetList(string type,string City,string School,string user,int channel_id, int category_id, int pageSize, int pageIndex, string strWhere, string filedOrder, out int recordCount)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            if (type == "3")
            {
                if (user != "")
                {
                    strSql.Append("  and user_name='" + user + "'");
                }
            }
            if (type == "4")
            {
                strSql.Append("  and SchoolId='" + School + "'");
            
            
            }
            if (type == "5")
            {
                strSql.Append("  and CityId='" + City + "'"+"and status=4");
        ...
        //普通老师获得的数据，获得是自己写的文章
        //public DataSet GetList(string user, int channel_id, int category_id, int pageSize, int pageIndex, string strWhere, string filedOrder, out int recordCount)
        //{
        //    StringBuilder strSql = new StringBuilder();
        //    strSql.Append("select * FROM " + databaseprefix + "article");
        //    if (channel_id > 0)
