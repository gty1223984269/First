...
namespace DTcms.Web.admin.article
{
    public partial class article_edit : Web.UI.ManagePage
    {
        ...
        private bool DoAdd()
        {
            ...
            model.user_name = Session["user"].ToString(); //获得当前登录用户名
            //model.CityId = int.Parse (Session["School"].ToString());
        ...
        private bool DoEdit(int _id)
        {
            ...
            model.sort_id = Utils.StrToInt(txtSortId.Text.Trim(), 99);
            model.click = int.Parse(txtClick.Text.Trim());
            //model.Sstatus = Utils.StrToInt(RBschool.SelectedValue, 0);
            //model.status = Utils.StrToInt(RBcity.SelectedValue, 0);
